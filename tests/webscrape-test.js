...
test("webscrape.handler", async t => {
  t.plan(1)
  if (process.env.RUN_SLOW != "y")
    return t.ok(true, "skipped! set RUN_SLOW=y to run this")
...
function mockEvent(message) {
  switch (message.arcType) {
    case "event":
      return {
        Records: [{ Sns: { Message: JSON.stringify(message.payload) } }]
      } // this is fine
    case "queue":
